...
export default function CheckoutModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!complete) {
      toast({
        title: "Ìyi shù kiàne",
        description: "Ìyi shù ulìtane yö́, wë̀ste",
        variant: "destructive",
      });
      return;
    }
    ...
    if (items.length === 0) {
      toast({
        title: "Kö̀l wa ìyi kë̀ tso'",
        description: "Ìyi kë̀ tso' be' kö̀l wa",
        variant: "destructive",
      });
      return;
    }
    ...
    if (storePhone && waWindow) {
      ...
      alert(`Pedido #${trackingNumber} registrado. La tienda te contactará al ${customerPhone}.`);
    } else {
    ...
    toast({
      title: "Patkène!",
      description: "Be' ìyi patkène WhatsApp wa",
    });
  };
  ...
  return (
    <Dialog open={showCheckout} onOpenChange={setShowCheckout}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl">Wablö̀</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="customerName">Be' kie</Label>
                <Input
                  id="customerName"
                  value={customerName}
                  onChange={(e) => setCustomerName(e.target.value)}
                  placeholder="Be' kie ulìtane"
                  required
                />
              </div>
              <div>
                <Label htmlFor="customerPhone">Telèfono</Label>
                <Input
                  id="customerPhone"
                  type="tel"
                  value={customerPhone}
